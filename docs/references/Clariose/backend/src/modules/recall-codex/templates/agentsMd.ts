// Builder for the per-user AGENTS.md auto-injected at the memories root.
// Codex picks this up via project settingSources at agent startup. The
// "Today" stats block is recomputed when the file is older than 1h.

export type AgentsMdInputs = {
  userIdHash: string;
  noteUploadsLast7d: number;
  recallSessionsLast7d: number;
  todayIso: string; // "YYYY-MM-DD"
};

export function buildAgentsMd(args: AgentsMdInputs): string {
  return `# Recall Coordinator workspace
...
- Date: ${args.todayIso}
- Recent note uploads (last 7d): ${args.noteUploadsLast7d}
- Recent recall sessions (last 7d): ${args.recallSessionsLast7d}
