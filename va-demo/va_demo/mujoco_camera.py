"""MuJoCo offscreen head-camera renderer for va-demo.

Mirror-image of the teleimager-backed `Camera` in `camera.py`, but the frames
come from MuJoCo's offscreen GL render of the simulated G1 head camera
instead of a USB image stream. Same public surface — `latest_bgr`,
`latest_jpeg_b64`, `frame_age_seconds`, `close` — so `realtime_agent` and
the safety watchdog can hold either object indistinguishably.

The simulator (e.g. unitree_mujoco's bridge or g1_sim_rl_combo.py) owns the
physics; we are a passive renderer running our own private MjModel/MjData
clone, syncing it to live joint positions (from `rt/lowstate`) and the
floating-base pose (from `rt/sportmodestate` when available, fallback to
identity), then offscreen-rendering RGB + depth.

We never write motor commands — only the bridge / skill server does.

Vendored from g1_brain/g1_brain/perception/mujoco_head_cam.py to keep
va-demo self-contained.
"""
...
# default -Z look direction onto the robot's +X (forward) axis.
...
class MuJoCoHeadCamera:
    ...
    def __init__(
        ...
        self._latest_base_quat: Optional[np.ndarray] = None
        self._latest_base_pos: Optional[np.ndarray] = None
        ...
        self._render_thread = threading.Thread(
            target=self._render_loop, name="va-demo-mujoco-head", daemon=True
        )
        ...
        *,
        ...
        if body is None:
            log.warning(
                ...
                "camera.attach_body to a real body name)",
                attach_body,
            )
            return spec.compile()
        ...
        log.info(
            "synthesized head camera %r on body %r at pos=%s fovy=%.1f deg",
    ...
    def _resolve_camera(self, name: str) -> int:
        ...
        if self._model.ncam > 0:
            ...
            return 0
        log.warning("no <camera> available; falling back to free camera at origin")
        return -1
